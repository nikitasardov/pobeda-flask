...
- Добавлен `docker compose restart frontend` в фазу деплоя —
  nginx не перечитывает конфиг без рестарта

**Подготовительные действия (выполнены):**
- Создан Gist с `coverage.json`, получен GIST_ID
- Создан PAT с правом `gist`, добавлен как GIST_TOKEN
- Настроен SSH-ключ, добавлены секреты SSH_KEY, SSH_HOST, SSH_USER, SSH_PORT, DEPLOY_PATH
- Репо клонирован на сервер в DEPLOY_PATH

**Результаты проверки:**
- Push в master → тесты и линт проходят, образ доставлен и развёрнут
- PR → запускаются только тесты (без деплоя)
- Badge покрытия отображается в README
- 3 фазы отображаются корректно в UI GitHub Actions
...
## Этап 12. Реструктуризация документации

**Что сделано:**
- Документация перенесена в `docs/`: чеклисты, DEVLOG, raw_task.txt
- Создан `docs/METHODOLOGY.md` — вынесены этапы, инструменты, требования, решения
- README.md переработан: компактная структура из 7 разделов вместо перегруженной
- Добавлены скриншоты UI в `docs/screenshots/` (4 скриншота основных сценариев)
- В README добавлено пояснение про `.env.example` и когда нужен `.env`
...
## Этап 13. Оптимизация Docker-сборки

**Что сделано:**
- Добавлен `.dockerignore` — из контекста сборки исключены docs, .github,
  .git, *.md и прочие файлы, не нужные приложению в контейнере

**Что скорректировано после ревью:**
- `.dockerignore` исключал `package.json`, `eslint.config.js`, `.htmlvalidate.json` —
  lint-сервис падал при сборке (`COPY package.json` not found).
  Файлы возвращены в контекст (общий `.dockerignore` для всех Dockerfile)

**Проблемы / корректировки:**
- `.dockerignore` один на весь build context — нельзя исключить файл
  для одного Dockerfile, но оставить для другого. Решение: оставить
  мелкие файлы (< 1 КБ) в контексте, исключать только тяжёлое
