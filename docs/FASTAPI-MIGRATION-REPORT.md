...
> Текущее состояние проекта: backend в корне репозитория уже переведён на FastAPI и используется как основная реализация.
...
## 1) Миграцию делаю в отдельной папке `FastAPIApp`, чтобы:
